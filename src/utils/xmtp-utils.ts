...
export async function createXmtpClientSafely(
  ...
  try {
    ...
    const client = await Promise.race([
      Client.create(sdkSigner, {
        env: env,
        // skipContactPublishing: false,  // Not supported in v5 create options directly?
        // persistConversations: true     // Not supported in v5 create options directly?
      }),
    ...
    console.log(`XMTP client created successfully on the ${env} network, client inboxId:`, client.inboxId);
    ...
    setTimeout(async () => {
      try {
        ...
        console.log("Retrieving conversations for user:", client.inboxId);
        ...
        if (conversations.length > 0) {
          ...
          for (const conversation of conversations) {
            try {
              ...
              const messages = await conversation.messages({ limit: 5n });
              const latestMessage = messages.length > 0 ? messages[messages.length - 1] : null;

              const peerId = 'dmPeerInboxId' in conversation ? (conversation as any).dmPeerInboxId : conversation.id;
              console.log(`Conversation with ${peerId}`);
              if (latestMessage) {
                console.log(`- Latest message: "${latestMessage.content}" (${new Date(Number(latestMessage.sentAtNs / 1000000n)).toLocaleString()})`);
                console.log(`- Sender: ${latestMessage.senderInboxId === client.inboxId ? 'You' : latestMessage.senderInboxId}`);
              } else {
              ...
              const peerId = 'dmPeerInboxId' in conversation ? (conversation as any).dmPeerInboxId : conversation.id;
              console.warn(`Error fetching messages for conversation with ${peerId}:`, msgErr);
            }
          }

          // Check if we can message a predefined test address
          try {
            // Note: canMessage in v5 might expect an Identifier object or array of Identifiers
            // For now, skipping this check or adapting it if possible.
            // const testAddress = "0x937C0d4a6294cdfa575de17382c7076b579DC176"; 
